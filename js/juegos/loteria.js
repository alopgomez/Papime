...
export function renderLoteria(container) {  
  container.innerHTML = `
    <div id="start-screenL">
        ...
        <button id="exitGameL">Salir</button>
    </div>

    <div style="align-items:center; text-align: center;">
    <button id="reset-game" class="btn">✖ Regresar al Menú</button>
  ...
  `;
    ...
    document.querySelectorAll('.level-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            document.getElementById('start-screenL').style.display = 'none';
            document.getElementById('reset-game').style.display = 'block'; // Mostrar al iniciar
            startGame(this.dataset.level);
        });
    });

    document.getElementById('reset-game').addEventListener('click', () => {
        clearTimeout(gameTimeout); // Detener el juego si regresan
        document.getElementById('start-screenL').style.display = 'flex';
        document.getElementById('reset-game').style.display = 'none'; // Ocultar en el menú
    });


    
  document.getElementById("exitGameL").
